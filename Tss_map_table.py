...
class Tss_map_table:
    ...
    def merge_all_tags(self):
        ...
        for tname in tableList:
            df = pd.read_sql_query("SELECT * FROM '{}' WHERE type='tag'".format(tname), con)
            tags.append(df)
        df_tags = pd.concat(tags)
        df_tags = df_tags.drop_duplicates()

        _, chrom, strand = tname.split('_')
        df_tags.to_sql('all_CAGE_tags_{}_{}'.format(chrom, strand), con, index=None, if_exists='replace')
        ...
        for cline in self.cell_lines:
            for chrom in self.chrom:
                for slabel, strand, tss_label in zip(['plus', 'minus'], ['+', '-'], ['start', 'end']):
                    ...
                    df_res.to_sql('{}_{}_{}'.format(cline, chrom, strand), con_out, index=None, if_exists='replace')
